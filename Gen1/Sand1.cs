...
namespace PKHeX.EncounterSlotDumper
{
    public static class Sand1
    {
        ...
        private static void FixSuperY(string name, int baseOffset)
        {
            ...
            while (data[i] != 0xFF)
            {
                ...
                for (int z = 0; z < count; z++)
                {
                    ...
                    int spc = data[ofs];
                    data[ofs] = (byte)GetG1Species(spc);
                }
                i += 2 * count;
            }
            File.WriteAllBytes(file + "fix", data);
        ...
        private static void FixEncounters(string name, int baseOffset)
        {
            ...
            string file = $"encounter_{name}.pkl";
            byte[] data = File.ReadAllBytes(file);
            ...
            for (var index = 0; index < offsets.Count; index++)
            {
                ...
                if (data[ofs++] != 0)
                {
                    for (int j = 0; j < 20; j += 2)
                        data[ofs + j + 1] = (byte) GetG1Species(data[ofs + j + 1]);
                    ofs += 20;
                }
        ...
        {
            byte[][] evos = new byte[252][];
            evos[0] = new byte[1];
            byte[][] learnsets = new byte[252][];
            ...
            using (var br = new BinaryReader(ms))
            {
                ...
                for (int i = 1; i < 252; i++)
                {
                    ...
                    List<byte> learn = new List<byte>();
                    while ((b = br.ReadByte()) != 0)
                        learn.Add(b);
                    learn.Add(0);

                    evos[i] = evo.Count > 0 ? evo.ToArray() : Array.Empty<byte>();
                    learnsets[i] = learn.Count > 0 ? learn.ToArray() : Array.Empty<byte>();
                }
                ...
                while (i < max - 1)
                {
                    ...
                    while ((b = br.ReadByte()) != 0)
                    {
                        ...
                        switch (b)
                        {
                            case 1:
                                evo.Add(br.ReadByte());
                                evo.Add((byte)GetG1Species(br.ReadByte()));
                                break;
                            case 2:
                                evo.Add(br.ReadByte());
                                evo.Add(br.ReadByte());
                    ...
                    if (index < 152)
                    {
                        evos[index] = evo.Count > 0 ? evo.ToArray() : Array.Empty<byte>();
                        learnsets[index] = learn.Count > 0 ? learn.ToArray() : Array.Empty<byte>();
                    }
                    i++;
                }
                Console.WriteLine((br.BaseStream.Position - ofs).ToString("X"));
        ...
        private static int GetG1Species(int raw_id) => itemTable[raw_id];

        private static readonly int[] itemTable =
        {
